...
{
	NVT_SetDeviceAddress(slave_addr);
	NVT_WriteByteContinue_addr8(reg_addr, data, length);
	return 0;
}
int i2c_read(unsigned char slave_addr, unsigned char reg_addr, unsigned char length, unsigned char * data)
{
	NVT_SetDeviceAddress(slave_addr);
	NVT_ReadByteContinue_addr8(reg_addr, data, length);
	return 0;
}
...
{
	CLK_SysTickLongDelay(num_ms *1000);
	return 0;
}
...
const struct test_s test = {
    32768/250, //.gyro_sens
    32768/2,   //.accel_sens : FSR = +-2G = 16384 LSB/G
    0,         //.reg_rate_div  (1kHz)
    2,         //.reg_lpf       (92Hz low pass filter)
    0,         //.reg_gyro_fsr  (250dps)
    0x0,       //.reg_accel_fsr (2g)
    200,       //.wait_ms       (200ms stabilization time)
    200,       //.packet_thresh (200 samples)
    20.f,      //.min_dps       (20 dps for Gyro Criteria C)
    60.f,      //.max_dps       (Must exceed 60 dps threshold for Gyro Criteria B)
    .5f,       //.max_gyro_var  (Must exceed +50% variation for Gyro Criteria A)
    .225f,     //.min_g         (Accel must exceed Min 225 mg for Criteria B)
    .675f,     //.max_g         (Accel cannot exceed Max 675 mg for Criteria B)
    .5f,       //.max_accel_var (Accel must be within 50% variation for Criteria A)
    .5f,       //.max_g_offset  (500 mg for Accel Criteria C)
    10         //.sample_wait_ms(10ms sample time wait)
};
