...
int MPU9250_updateAccel(void)
{
	...
	if (mpu_get_accel_reg(data, &imu.time))
	{
		return INV_ERROR;		
	}
	imu.ax = data[X_AXIS];
...
int MPU9250_updateGyro(void)
{
	...
	if (mpu_get_gyro_reg(data, &imu.time))
	{
		return INV_ERROR;		
	}
	imu.gx = data[X_AXIS];
...
int MPU9250_updateCompass(void)
{
	...
	if (mpu_get_compass_reg(data, &imu.time))
	{
		return INV_ERROR;		
	}
	imu.mx = data[X_AXIS];
...
{
	return mpu_get_temperature(&imu.temperature, &imu.time);
}
...
{
    float dqw = qToFloat(imu.qw, 30);
    float dqx = qToFloat(imu.qx, 30);
    float dqy = qToFloat(imu.qy, 30);
    float dqz = qToFloat(imu.qz, 30);
...
{
	if (imu.my == 0)
		imu.heading = (imu.mx < 0) ? PI : 0;
	else
		imu.heading = atan2(imu.mx, imu.my);
	
	if (imu.heading > PI) imu.heading -= (2 * PI);
	else if (imu.heading < -PI) imu.heading += (2 * PI);
	else if (imu.heading < 0) imu.heading += 2 * PI;
	
	imu.heading*= 180.0 / PI;
